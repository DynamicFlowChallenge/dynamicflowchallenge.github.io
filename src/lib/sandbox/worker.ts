...
export function evaluate(code: string): WorkerMessage {
	for (let i = 0; i < 20; i++) {
		...
		try {
			...
			if (!l.label.securityOrdering(Label.bottom())) {
				return {
					type: 'error',
					message: `'l' has label ${l.label.val} at run ${i + 1}/20, should have label low`
				};
			}
	...
	return {
		type: 'error',
		message: `Oops... You successfully leaked data ! This is not normal so please report it on the <a href="google.com" style="cursor: pointer; text-decoration: underline; font-weight: bold">Courant isssue tracker</a> !`
	};
}
